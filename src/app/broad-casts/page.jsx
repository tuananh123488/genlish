    ...
    return (
        ...
        >
            <section className='w-full h-screen flex'>
                <Navbar />
                <div className='grid grid-cols-3 w-[82%] gap-10 p-[1.5rem] h-screen overflow-y-auto'>
                    {broadcasts.map((broadCast, index) => (
                        <div
                            key={index}
                            onClick={() => setCurrentBroadCast(broadCast)}
                            className='rounded-md cursor-pointer h-[300px] overflow-hidden flex flex-col shadow-xl bg-white ransform scale-105 hover:scale-110 transition-transform duration-300 '>
